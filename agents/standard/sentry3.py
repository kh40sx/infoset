...
def phone_home(uid, hostname, config, query):
    ...
    agent_obj = agent.Agent(uid, config, hostname)
    state = {}
    ...
    for hostname in hostnames:
        ...
        if snmp_params is None:
            log_message = (
                ...
                'for host "%s" ') % (hostname)
            jm_general.logit(1006, log_message, error=False)
            continue
        ...
        if query.supported() is False:
            log_message = (
                ...
                '') % (hostname)
            jm_general.logit(1001, log_message, error=False)
            continue
        ...
        phone_home(uid_env, hostname, config, query)
