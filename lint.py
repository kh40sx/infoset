...
class functionObject:
	...
	#The linter uses the indent level to check if the function being checked has started/ended
	...
	def isDone(self, currentLine = ""):
		...
		if currentLine != "" and currentLine != "\n" and currentLine != "\t":
			...
			#len(currentLine.lstrip() : length of line without leading whitespace, i.e. tabs and spaces
			#(len(currentLine) - len(currentLine.lstrip())) : the indent level of the current line
			#self.indentLevel represents the number of indents that preceded the function signature.
			#if the indent level of the current line is the same as that of the function signature, the function is ended
	...
	def parseDocStringData(self):
		...
		Before this function was called, all text found within the scanned function's docstring
		...
		for tokenized in self.docStringText:
			stripped = tokenized.string.strip() #Strips all preceding and following whitespace characters
			...
			if stripped == ":" and previousToken == "Args":
				...
				if argsPrecedingSpaces == currentPrecedingSpaces:
					#If the current line's indentation level is the same as the start of Args...
					argsSectionStarted = False
					#print("Args section done")
				elif len(stripped) > 0 and tokenized.string == ":":
					#While in the Args section, add anything in front of a : to the list of arguments.
		...
		try:
			for tokenized in token_generator:
				...
				if func.startedDocString and func.hasFoundDef():
					...
					#If the current token is "def", then this is the start of a function
					#Sets the hasFoundDef flag, which basically says the start
					#of a function has been found
					#Also records the indentation level of the function signature
					...
					#Appends to the list of function arguments until a ) is encountered
					if tokenized.string == ")":
					...
					#Adds anything found after "return" to the list of function arguments
